...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    // 확산광
    float diffuseFactor = max(dot(lightDir, normal), 0.0f); // 빛의 양 계산
    float3 diffuse = lightColor.rgb * lightColor.a * diffuseFactor; // 빛의 색상 * 빛의 강도 * 빛의 양
    
    // 반사광
    float3 reflection = reflect(-lightDir, normal); // 반사광 계산
    float specularFactor = pow(max(dot(reflection, toEye), 0.0f) /*반사광의 양 계산*/, 16.0f); // 제곱은 광택 정도
